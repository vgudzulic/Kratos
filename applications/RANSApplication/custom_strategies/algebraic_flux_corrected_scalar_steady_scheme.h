...
private:
    ...
    template <typename TItem>
    void AddAntiDiffusiveFluxes(Vector& rRHS, Matrix& rLHS, const TItem& rItem, const Matrix& rArtificialDiffusion)
    {
        Vector values;
        rItem.GetValuesVector(values);
    ...
    void AddLumpedMassMatrix(Matrix& rLHS, TItem& rItem, const ProcessInfo& rCurrentProcessInfo) const
    {
        ...
        double residual = 0.0;
        rItem.Calculate(RESIDUAL, residual, rCurrentProcessInfo);
