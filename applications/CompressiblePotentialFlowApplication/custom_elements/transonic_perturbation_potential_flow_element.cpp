    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    const array_1d<double, 3> free_stream_velocity =
        rCurrentProcessInfo[FREE_STREAM_VELOCITY];
...
int TransonicPerturbationPotentialFlowElement<TDim, TNumNodes>::GetAdditionalUpwindNodeIndex() const {
    ...
    KRATOS_ERROR_IF(!upstream_element_id_found)
        << "No upstream element id found for element #" << this->Id() << std::endl;
