...
#include "bed_swimming_particle.h"
...
BedSwimmingParticle<TBaseElement>& BedSwimmingParticle<TBaseElement>::operator=(const BedSwimmingParticle<TBaseElement>& rOther) {
    ...
    mNSteady = rOther.mNSteady;
    mMinFluidFrac = rOther.mMinFluidFrac;
...
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::CreateHydrodynamicInteractionLaws(const ProcessInfo& r_process_info)
{
    mHydrodynamicInteractionLaw = this->GetProperties()[SDEM_HYDRODYNAMIC_INTERACTION_LAW_POINTER]->Clone();
}

template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::ComputeAdditionalForces(array_1d<double, 3>& non_contact_force,
...
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::UpdateNodalValues(NodeType& node,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::ApplyNumericalAveragingWithOldForces(NodeType& node, array_1d<double, 3>& non_contact_nor_history_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::AdditionalCalculate(const Variable<double>& rVariable, double& Output, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::Calculate(const Variable<array_1d<double, 3 > >& rVariable,
                                               ...
                                               const ProcessInfo& r_current_process_info)
{
    ...
    NodeType& node = GetGeometry()[0];
    bool must_be_erased = CheckToErase(r_current_process_info);
    node.Set(TO_ERASE, must_be_erased);
}
//**************************************************************************************************************************************************
//**************************************************************************************************************************************************
template < class TBaseElement >
double BedSwimmingParticle<TBaseElement>::CalculateDragCoeffFromSphericity(const double reynolds,
...
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::ApplyDragPorosityModification(double& drag_coeff)
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
double BedSwimmingParticle<TBaseElement>::GetFluidMass()
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
array_1d<double,3> BedSwimmingParticle<TBaseElement>::ComputeWeight(const array_1d<double,3>& gravity, const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
void BedSwimmingParticle<TBaseElement>::AddCentrifugalForces(array_1d<double,3>& weight, const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
void BedSwimmingParticle<TBaseElement>::AddCoriolisForces(array_1d<double,3>& weight, const ProcessInfo& r_process_info)
{
    const array_1d<double,3>& omega = r_process_info[ANGULAR_VELOCITY_MOVING_FRAME];
...
//**************************************************************************************************************************************************
template< class TBaseElement >
void BedSwimmingParticle<TBaseElement>::AddRelativeAccelerationForces(array_1d<double,3>& weight, const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
void BedSwimmingParticle<TBaseElement>::AddEulerForces(array_1d<double,3>& weight, const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template< class TBaseElement >
void BedSwimmingParticle<TBaseElement>::Initialize(const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void BedSwimmingParticle<TBaseElement>::MemberDeclarationFirstStep(const ProcessInfo& r_process_info)
{
    ...
    mNSteady                      = r_process_info[MAX_STEADY_STEPS];
    mMinFluidFrac                 = r_process_info[MIN_FLUID_FRACTION_PROJECTED];
}
//**************************************************************************************************************************************************
//**************************************************************************************************************************************************
template< class TBaseElement >
bool BedSwimmingParticle<TBaseElement>::CheckToErase(const ProcessInfo& r_process_info)
{
    NodeType& node = GetGeometry()[0];
    const array_1d<double, 3>& particle_vel = node.FastGetSolutionStepValue(VELOCITY);
    int n_steps_steady, n_steady = mNSteady;
    double min_fluid_frac_projected = mMinFluidFrac;

    if (SWIMMING_MODULUS_3(particle_vel) < 1e-6){
        n_steps_steady++;
    }else{
        n_steps_steady=0;
    }

    if (n_steps_steady >= n_steady || node.FastGetSolutionStepValue(FLUID_FRACTION_PROJECTED) < min_fluid_frac_projected){
        return true;
    }else{
        return false;
    }
...
template class BedSwimmingParticle<SphericParticle>;
