...
void SplitForwardEulerSphericParticle::InitializeForceComputation(ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::FirstCalculateRightHandSide(ProcessInfo& r_process_info, double dt, int search_control){}

void SplitForwardEulerSphericParticle::CollectCalculateRightHandSide(ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::FinalCalculateRightHandSide(ProcessInfo& r_process_info, double dt, const array_1d<double,3>& gravity){}

void SplitForwardEulerSphericParticle::CalculateMaxBallToBallIndentation(double& r_current_max_indentation, const ProcessInfo& r_process_info)
{
...
void SplitForwardEulerSphericParticle::CalculateMaxBallToFaceIndentation(double& r_current_max_indentation)
{
    r_current_max_indentation = - std::numeric_limits<double>::max();
...
void SplitForwardEulerSphericParticle::CalculateMomentum(array_1d<double, 3>& r_momentum)
{
    const array_1d<double, 3>& vel = this->GetGeometry()[0].FastGetSolutionStepValue(VELOCITY);
    noalias(r_momentum) = GetMass() * vel;
}

void SplitForwardEulerSphericParticle::CalculateLocalAngularMomentum(array_1d<double, 3>& r_angular_momentum)
{
...
void SplitForwardEulerSphericParticle::ComputeNewNeighboursHistoricalData(DenseVector<int>& temp_neighbours_ids,
...
void SplitForwardEulerSphericParticle::ComputeNewRigidFaceNeighboursHistoricalData()
{
    array_1d<double, 3> vector_of_zeros = ZeroVector(3);
    std::vector<DEMWall*>& rNeighbours = this->mNeighbourRigidFaces;
...
void SplitForwardEulerSphericParticle::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::CalculateMassMatrix(MatrixType& rMassMatrix, ProcessInfo& r_process_info)
{
    rMassMatrix(0,0) = GetMass();
}

void SplitForwardEulerSphericParticle::EvaluateDeltaDisplacement(ParticleDataBuffer & data_buffer,
...
void SplitForwardEulerSphericParticle::RelativeDisplacementAndVelocityOfContactPointDueToRotation(const double indentation,
                                                double RelDeltDisp[3],
                                                double RelVel[3],
...
void SplitForwardEulerSphericParticle::RelativeDisplacementAndVelocityOfContactPointDueToRotationMatrix(double DeltDisp[3],
                                                double RelVel[3],
...
void SplitForwardEulerSphericParticle::RelativeDisplacementAndVelocityOfContactPointDueToRotationQuaternion(double RelDeltDisp[3],
                                                double RelVel[3],
                                                const double OldLocalCoordSystem[3][3],
                                                const double& other_radius,
                                                const double& dt,
...
void SplitForwardEulerSphericParticle::ComputeMoments(double NormalLocalContactForce,
                                     double Force[3],
...
void SplitForwardEulerSphericParticle::ComputeRollingResistance(double& RollingResistance, const double& NormalLocalContactForce, const double& equiv_rolling_friction_coeff, const unsigned int i) {
...
void SplitForwardEulerSphericParticle::ComputeRollingFriction(array_1d<double, 3>& rolling_resistance_moment, double& RollingResistance, double dt)
{
...
void SplitForwardEulerSphericParticle::ComputeBallToBallContactForce(SphericParticle::ParticleDataBuffer & data_buffer,
...
void SplitForwardEulerSphericParticle::RenewData()
{
  //To be redefined
}
void SplitForwardEulerSphericParticle::ComputeConditionRelativeData(int rigid_neighbour_index,   // check for delete
                                                    DEMWall* const wall,
                                                    double LocalCoordSystem[3][3],
                                                    double& DistPToB,
...
void SplitForwardEulerSphericParticle::ComputeWear(double LocalRelVel[3],
                                  double mTimeStep,
                                  bool sliding,
...
void SplitForwardEulerSphericParticle::CreateDiscontinuumConstitutiveLaws(const ProcessInfo& r_process_info)
{
...
void SplitForwardEulerSphericParticle::CalculateDampingMatrix(MatrixType& rDampingMatrix, ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::GetDofList(DofsVectorType& ElementalDofList, ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
void SplitForwardEulerSphericParticle::InitializeSolutionStep(ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
void SplitForwardEulerSphericParticle::AddNeighbourContributionToStressTensor(ProcessInfo& r_process_info,
...
void SplitForwardEulerSphericParticle::AddWallContributionToStressTensor(const double Force[3],
                                                        const double other_to_me_vect[3],
                                                        const double distance,
...
void SplitForwardEulerSphericParticle::CorrectRepresentativeVolume(double& rRepresentative_Volume/*, bool& is_smaller_than_sphere*/) {
...
void SplitForwardEulerSphericParticle::FinalizeSolutionStep(ProcessInfo& r_process_info){
...
void SplitForwardEulerSphericParticle::SymmetrizeStressTensor(){
...
void SplitForwardEulerSphericParticle::ComputeReactions() {
...
void SplitForwardEulerSphericParticle::PrepareForPrinting(ProcessInfo& r_process_info){
...
void SplitForwardEulerSphericParticle::ComputeAdditionalForces(array_1d<double, 3>& externally_applied_force,
...
array_1d<double,3> SplitForwardEulerSphericParticle::ComputeWeight(const array_1d<double,3>& gravity, const ProcessInfo& r_process_info) {
...
void SplitForwardEulerSphericParticle::MemberDeclarationFirstStep(const ProcessInfo& r_process_info)
...
double SplitForwardEulerSphericParticle::CalculateLocalMaxPeriod(const bool has_mpi, const ProcessInfo& r_process_info) {
    KRATOS_TRY
...
void SplitForwardEulerSphericParticle::ComputeOtherBallToBallForces(array_1d<double, 3>& other_ball_to_ball_forces) {}


double SplitForwardEulerSphericParticle::GetInitialDeltaWithFEM(int index) //only available in continuum_particle
{
    return 0.0;
}

void SplitForwardEulerSphericParticle::Calculate(const Variable<double>& rVariable, double& Output, const ProcessInfo& r_process_info)
{
    KRATOS_TRY
...
void SplitForwardEulerSphericParticle::Calculate(const Variable<array_1d<double, 3> >& rVariable, array_1d<double, 3>& Output,
                                const ProcessInfo& r_process_info)
{
...
void SplitForwardEulerSphericParticle::RotateOldContactForces(const double OldLocalCoordSystem[3][3], const double LocalCoordSystem[3][3], array_1d<double, 3>& mNeighbourElasticContactForces) {
...
bool SplitForwardEulerSphericParticle::CalculateRelativePositionsOrSkipContact(ParticleDataBuffer & data_buffer)
{
...
void SplitForwardEulerSphericParticle::RelativeDisplacementAndVelocityOfContactPointDueToOtherReasons(const ProcessInfo& r_process_info,
...
void SplitForwardEulerSphericParticle::SendForcesToFEM(){}

void SplitForwardEulerSphericParticle::TransformNeighbourCoorsToClosestInPeriodicDomain(ParticleDataBuffer & data_buffer)
{
...
void SplitForwardEulerSphericParticle::TransformNeighbourCoorsToClosestInPeriodicDomain(const ProcessInfo& r_process_info,
...
void SplitForwardEulerSphericParticle::Move(const double delta_t, const bool rotation_option, const double force_reduction_factor, const int StepFlag) {
...
bool SplitForwardEulerSphericParticle::SwapIntegrationSchemeToGluedToWall(Condition* p_wall) {
...
void SplitForwardEulerSphericParticle::Calculate(const Variable<Vector >& rVariable, Vector& Output, const ProcessInfo& r_process_info){}
void SplitForwardEulerSphericParticle::Calculate(const Variable<Matrix >& rVariable, Matrix& Output, const ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::AdditionalCalculate(const Variable<double>& rVariable, double& Output, const ProcessInfo& r_process_info){}

void SplitForwardEulerSphericParticle::ApplyGlobalDampingToContactForcesAndMoments(array_1d<double,3>& total_forces, array_1d<double,3>& total_moment) {
    ...
    void SplitForwardEulerSphericParticle::CalculateOnContactElements(size_t i, double LocalContactForce[3]) {
...
int    SplitForwardEulerSphericParticle::GetClusterId()                                                    { return mClusterId;      }
void   SplitForwardEulerSphericParticle::SetClusterId(int givenId)                                         { mClusterId = givenId;   }
double SplitForwardEulerSphericParticle::GetRadius()                                                       { return mRadius;         }
double SplitForwardEulerSphericParticle::CalculateVolume()                                                 { return 4.0 * Globals::Pi / 3.0 * mRadius * mRadius * mRadius;     }
void   SplitForwardEulerSphericParticle::SetRadius(double radius)                                          { mRadius = radius;       }
void   SplitForwardEulerSphericParticle::SetRadius()                                                       { mRadius = GetGeometry()[0].FastGetSolutionStepValue(RADIUS);       }
double SplitForwardEulerSphericParticle::GetInteractionRadius(const int radius_index)                      { return mRadius;         }
void   SplitForwardEulerSphericParticle::SetInteractionRadius(const double radius, const int radius_index) { mRadius = radius; GetGeometry()[0].FastGetSolutionStepValue(RADIUS) = radius;}
double SplitForwardEulerSphericParticle::GetSearchRadius()                                                 { return mSearchRadius;   }
void   SplitForwardEulerSphericParticle::SetSearchRadius(const double radius)                              { mSearchRadius = radius; }
void SplitForwardEulerSphericParticle::SetDefaultRadiiHierarchy(const double radius)
{
    SetRadius(radius);
    SetSearchRadius(radius);
}

double SplitForwardEulerSphericParticle::GetMass()                                                         { return mRealMass;       }
void   SplitForwardEulerSphericParticle::SetMass(double real_mass)                                         { mRealMass = real_mass;  GetGeometry()[0].FastGetSolutionStepValue(NODAL_MASS) = real_mass;}
double SplitForwardEulerSphericParticle::CalculateMomentOfInertia()                                        { return 0.4 * GetMass() * GetRadius() * GetRadius(); }

double SplitForwardEulerSphericParticle::GetYoung()                                                        { return GetFastProperties()->GetYoung();                    }
double SplitForwardEulerSphericParticle::GetRollingFriction()                                              { return GetFastProperties()->GetRollingFriction();          }
double SplitForwardEulerSphericParticle::GetRollingFrictionWithWalls()                                     { return GetFastProperties()->GetRollingFrictionWithWalls(); }
double SplitForwardEulerSphericParticle::GetPoisson()                                                      { return GetFastProperties()->GetPoisson();                  }
double SplitForwardEulerSphericParticle::GetTgOfStaticFrictionAngle()                                      { return GetFastProperties()->GetTgOfStaticFrictionAngle();  }
double SplitForwardEulerSphericParticle::GetTgOfDynamicFrictionAngle()                                     { return GetFastProperties()->GetTgOfDynamicFrictionAngle(); }
double SplitForwardEulerSphericParticle::GetCoefficientOfRestitution()                                     { return GetFastProperties()->GetCoefficientOfRestitution(); }
double SplitForwardEulerSphericParticle::GetLnOfRestitCoeff()                                              { return GetFastProperties()->GetLnOfRestitCoeff();          }
double SplitForwardEulerSphericParticle::GetDensity()                                                      { return GetFastProperties()->GetDensity();                  }
int    SplitForwardEulerSphericParticle::GetParticleMaterial()                                             { return GetFastProperties()->GetParticleMaterial();         }
double SplitForwardEulerSphericParticle::GetParticleCohesion()                                             { return GetFastProperties()->GetParticleCohesion();         }
double SplitForwardEulerSphericParticle::GetParticleKNormal()                                              { return GetFastProperties()->GetParticleKNormal();          }
double SplitForwardEulerSphericParticle::GetParticleKTangential()                                          { return GetFastProperties()->GetParticleKTangential();      }

array_1d<double, 3>& SplitForwardEulerSphericParticle::GetForce()                                          { return GetGeometry()[0].FastGetSolutionStepValue(TOTAL_FORCES);}
double&              SplitForwardEulerSphericParticle::GetElasticEnergy()                                  { return mElasticEnergy; }
double&              SplitForwardEulerSphericParticle::GetInelasticFrictionalEnergy()                      { return mInelasticFrictionalEnergy; }
double&              SplitForwardEulerSphericParticle::GetInelasticViscodampingEnergy()                    { return mInelasticViscodampingEnergy; }

void   SplitForwardEulerSphericParticle::SetYoungFromProperties(double* young)                                          { GetFastProperties()->SetYoungFromProperties( young);                                            }
void   SplitForwardEulerSphericParticle::SetRollingFrictionFromProperties(double* rolling_friction)                     { GetFastProperties()->SetRollingFrictionFromProperties( rolling_friction);                       }
void   SplitForwardEulerSphericParticle::SetRollingFrictionWithWallsFromProperties(double* rolling_friction_with_walls) { GetFastProperties()->SetRollingFrictionWithWallsFromProperties( rolling_friction_with_walls);   }
void   SplitForwardEulerSphericParticle::SetPoissonFromProperties(double* poisson)                                      { GetFastProperties()->SetPoissonFromProperties( poisson);                                        }
void   SplitForwardEulerSphericParticle::SetTgOfStaticFrictionAngleFromProperties(double* tg_of_static_friction_angle)  { GetFastProperties()->SetTgOfStaticFrictionAngleFromProperties( tg_of_static_friction_angle);    }
void   SplitForwardEulerSphericParticle::SetTgOfDynamicFrictionAngleFromProperties(double* tg_of_dynamic_friction_angle){ GetFastProperties()->SetTgOfDynamicFrictionAngleFromProperties( tg_of_dynamic_friction_angle);}
void   SplitForwardEulerSphericParticle::SetCoefficientOfRestitutionFromProperties(double* coefficient_of_restitution)  { GetFastProperties()->SetCoefficientOfRestitutionFromProperties( coefficient_of_restitution);    }
void   SplitForwardEulerSphericParticle::SetLnOfRestitCoeffFromProperties(double* ln_of_restit_coeff)                   { GetFastProperties()->SetLnOfRestitCoeffFromProperties( ln_of_restit_coeff);                     }
void   SplitForwardEulerSphericParticle::SetDensityFromProperties(double* density)                                      { GetFastProperties()->SetDensityFromProperties( density);                                        }
void   SplitForwardEulerSphericParticle::SetParticleMaterialFromProperties(int* particle_material)                      { GetFastProperties()->SetParticleMaterialFromProperties( particle_material);                     }
void   SplitForwardEulerSphericParticle::SetParticleCohesionFromProperties(double* particle_cohesion)                   { GetFastProperties()->SetParticleCohesionFromProperties( particle_cohesion);                     }
void   SplitForwardEulerSphericParticle::SetParticleKNormalFromProperties(double* particle_k_normal)                    { GetFastProperties()->SetParticleKNormalFromProperties( particle_k_normal);                      }
void   SplitForwardEulerSphericParticle::SetParticleKTangentialFromProperties(double* particle_k_tangential)            { GetFastProperties()->SetParticleKTangentialFromProperties( particle_k_tangential);              }

DEMDiscontinuumConstitutiveLaw::Pointer SplitForwardEulerSphericParticle::GetConstitutiveLawPointer()      { return mDiscontinuumConstitutiveLaw; }


PropertiesProxy* SplitForwardEulerSphericParticle::GetFastProperties()                                     { return mFastProperties;   }
void   SplitForwardEulerSphericParticle::SetFastProperties(PropertiesProxy* pProps)                        { mFastProperties = pProps; }
void   SplitForwardEulerSphericParticle::SetFastProperties(std::vector<PropertiesProxy>& list_of_proxies)  {
...
double SplitForwardEulerSphericParticle::SlowGetYoung() const                    { return GetProperties()[YOUNG_MODULUS];               }
double SplitForwardEulerSphericParticle::SlowGetRollingFriction() const          { return GetProperties()[ROLLING_FRICTION];            }
double SplitForwardEulerSphericParticle::SlowGetRollingFrictionWithWalls() const { return GetProperties()[ROLLING_FRICTION_WITH_WALLS]; }
double SplitForwardEulerSphericParticle::SlowGetPoisson() const                  { return GetProperties()[POISSON_RATIO];               }
double SplitForwardEulerSphericParticle::SlowGetTgOfStaticFrictionAngle() const  { return GetProperties()[STATIC_FRICTION];             }
double SplitForwardEulerSphericParticle::SlowGetTgOfDynamicFrictionAngle() const { return GetProperties()[DYNAMIC_FRICTION];            }
double SplitForwardEulerSphericParticle::SlowGetCoefficientOfRestitution() const { return GetProperties()[COEFFICIENT_OF_RESTITUTION];  }
double SplitForwardEulerSphericParticle::SlowGetDensity() const                  { return GetProperties()[PARTICLE_DENSITY];            }
int    SplitForwardEulerSphericParticle::SlowGetParticleMaterial() const         { return GetProperties()[PARTICLE_MATERIAL];           }
double SplitForwardEulerSphericParticle::SlowGetParticleCohesion() const         { return GetProperties()[PARTICLE_COHESION];           }
