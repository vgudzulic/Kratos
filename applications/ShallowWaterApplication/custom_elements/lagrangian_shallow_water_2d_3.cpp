...
#include "lagrangian_shallow_water_2d_3.h"

namespace Kratos
{

int LagrangianShallowWater2D3::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void LagrangianShallowWater2D3::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void LagrangianShallowWater2D3::GetDofList(DofsVectorType& rElementalDofList,ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void LagrangianShallowWater2D3::CalculateLocalSystem(
    ...
    ProcessInfo& rCurrentProcessInfo)
{
    ...
    ElementData data(rCurrentProcessInfo);
...
void LagrangianShallowWater2D3::AddInertiaTerms(
    MatrixType& rLHS,
    VectorType& rRHS,
...
void LagrangianShallowWater2D3::AddGradientTerms(
    MatrixType& rLHS,
    VectorType& rRHS,
    const ElementData& rData,
    const array_1d<double,3>& rN,
...
void LagrangianShallowWater2D3::AddSourceTerms(
    MatrixType& rLHS,
    VectorType& rRHS,
    const ElementData& rData,
    const array_1d<double,3>& rN,
    const BoundedMatrix<double,3,2>& rDN_DX)
{
    ...
    const double height4_3 = std::pow(rData.height, 2.33333333333) + 1e-3;
...
void LagrangianShallowWater2D3::AddShockCapturingTerm(
    MatrixType& rLHS,
    const ElementData& rData,
    const BoundedMatrix<double,3,2>& rDN_DX)
{
...
void LagrangianShallowWater2D3::CalculateLeftHandSide(
    MatrixType& rLeftHandSideMatrix,
    ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_ERROR << "LagrangianShallowWater2D3: CalculateLeftHandSide not implemented" << std::endl;
}

void LagrangianShallowWater2D3::CalculateRightHandSide(
    VectorType& rRightHandSideVector,
    ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_ERROR << "LagrangianShallowWater2D3: CalculateRightHandSide not implemented" << std::endl;
}

void LagrangianShallowWater2D3::GetValueOnIntegrationPoints(
...
LagrangianShallowWater2D3::ElementData::ElementData(const ProcessInfo& rCurrentProcessInfo)
{
...
void LagrangianShallowWater2D3::ElementData::GetNodalData(const GeometryType& rGeometry, const BoundedMatrix<double,3,2>& rDN_DX)
{
    height = 0.0;
    ...
    for (unsigned int i = 0; i < 3; i++)
    {
        ...
        const auto q = rGeometry[i].FastGetSolutionStepValue(MOMENTUM);
        const auto hn = rGeometry[i].FastGetSolutionStepValue(HEIGHT, 1);
        const auto qn = rGeometry[i].FastGetSolutionStepValue(MOMENTUM, 1);
        ...
        pressure_a[i] = rGeometry[i].FastGetSolutionStepValue(ATMOSPHERIC_PRESSURE);
...
void LagrangianShallowWater2D3::ComputeMassMatrix(
    ...
    const BoundedMatrix<double,3,2>& rDN_DX)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        for (size_t j = 0; j < 3; ++j)
        {
            ...
            // rMatrix(i_block,     j_block + 2) += l * cmm * mu_q * s1_ij * g;
            // rMatrix(i_block + 2, j_block)     += l * cmm * mu_h * s1_ij * h;
            //
            //  /* Stabilization y
            //   * A2*M
            //   */
            // const double s2_ij = rN[j] * rDN_DX(i,1);
            // rMatrix(i_block + 1, j_block + 2) += l * cmm * mu_q * s2_ij * g;
            // rMatrix(i_block + 2, j_block + 1) += l * cmm * mu_h * s2_ij * h;
        }
    }
}
...
void LagrangianShallowWater2D3::ComputeGradientMatrix(
    BoundedMatrix<double,9,9>& rMatrix,
    const ElementData& rData,
    const array_1d<double,3>& rN,
    const BoundedMatrix<double,3,2>& rDN_DX)
{
    using std::pow;
    const double g = rData.gravity;
    const double h = (rData.height < 1e-2) ? 0.0 : rData.height;
    const double c2 = g*h; // c = sqrt(gh)
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        for (size_t j = 0; j < 3; ++j)
        {
            ...
            rMatrix(i_block + 2, j_block)     += g1_ij * 1.0;
            ...
            rMatrix(i_block + 2, j_block + 1) += g2_ij * 1.0;
            ...
            rMatrix(i_block,     j_block)     += l * d_ij * c;
            rMatrix(i_block + 2, j_block + 2) += l * d_ij * c;

            /* Stabilization y-y
             * A2*A2
             */
            d_ij = rDN_DX(j,1) * rDN_DX(i,1);
            rMatrix(i_block + 1, j_block + 1) += l * d_ij * c;
            rMatrix(i_block + 2, j_block + 2) += l * d_ij * c;
            ...
            rMatrix(i_block,     j_block + 1) += l * d_ij * c;

            /* Stabilization y-x
             * A2*A1
             */
            d_ij = rDN_DX(j,1) * rDN_DX(i,0);
            rMatrix(i_block + 1, j_block)     += l * d_ij * c;
        }
    }
}

void LagrangianShallowWater2D3::ComputeDiffusionMatrix(
    BoundedMatrix<double,9,9>& rMatrix,
    const ElementData& rData,
    const BoundedMatrix<double,3,2>& rDN_DX,
...
void LagrangianShallowWater2D3::ComputeGradientVector(
    ...
    const BoundedMatrix<double,3,2>& rDN_DX)
{
    ...
    const double c2 = g*h;
    const double c = std::sqrt(c2);
    const double r = rData.density;
    const double l = StabilizationParameter(rData);
    const auto depth = rData.depth;
    const auto p_a = rData.pressure_a;
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        {
            /* First component */
            rVector[i_block] -= rN[i] * rDN_DX(j,0) * (c2*depth[j] + h*p_a[j]/r);

            /* Second component */
            rVector[i_block + 1] -= rN[i] * rDN_DX(j,1) * (c2*depth[j] + h*p_a[j]/r);

            /* Stabilization x-x
             * A1*G1
             */
            rVector[i_block + 2] -= l * c *rDN_DX(i,0) * rDN_DX(j,0) * (c*depth[j] + h*p_a[j]/r/(c+1e-2));

            /* Stabilization y-y
             * A2*G2
             */
            rVector[i_block + 2] -= l * c * rDN_DX(i,1) * rDN_DX(j,1) * (c*depth[j] + h*p_a[j]/r/(c+1e-2));
...
void LagrangianShallowWater2D3::ComputeCrossWindDiffusivityTensors(
    ...
    const BoundedMatrix<double,3,2>& rDN_DX)
{
    ...
    const double k = 0.5 * rData.shock_stab_factor * length * std::abs(height_residual) / h_grad;
    rKh = k * cross_wind;
}

void LagrangianShallowWater2D3::AlgebraicResidual(
    ...
    const BoundedMatrix<double,3,3> rFlowGrad)
{
    ...
    double rain = 0.0;
    array_1d<double,3> press_grad = ZeroVector(3);// Todo: compute it
    ...
    const double g = rData.gravity;
    const array_1d<double,3> friction = g * rData.manning2 * norm_2(rData.velocity) * rData.velocity / (std::pow(rData.height, 0.333333333333333) + 1e-6);
    const array_1d<double,3> press = rData.height * press_grad / rData.density;

    rFlowResidual = flow_acc + g * rHeightGrad + friction + press;
    rHeightresidual = height_acc + rData.height * rFlowDiv + rain;
}

void LagrangianShallowWater2D3::StreamLineTensor(BoundedMatrix<double,2,2>& rTensor, const array_1d<double,3>& rVector)
{
...
void LagrangianShallowWater2D3::CrossWindTensor(BoundedMatrix<double,2,2>& rTensor, const array_1d<double,3>& rVector)
{
    const double e = std::numeric_limits<double>::epsilon(); // small value to avoid division by zero
    const auto aux_vector = static_cast<array_1d<double,2>>(rVector);
...
double LagrangianShallowWater2D3::StabilizationParameter(const ElementData& rData)
{
    const double e = std::numeric_limits<double>::epsilon(); // small value to avoid division by zero
    const double length = this->GetGeometry().Length();
    const double eigenvalue = 1.0;//std::sqrt(rData.gravity * rData.height);
    return length * rData.stab_factor / (eigenvalue + e);
}

array_1d<double,3> LagrangianShallowWater2D3::CharacteristicLength(const ElementData& rData)
{
    const double e = std::numeric_limits<double>::epsilon(); // small value to avoid division by zero
    const double length = this->GetGeometry().Length();
    const double char_length = 0.5 * length * rData.stab_factor;
    return char_length * rData.velocity / (norm_2(rData.velocity) + e);
}
