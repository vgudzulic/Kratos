...
class MmgProcess(KratosMultiphysics.Process):
    ...
    def ExecuteFinalize(self):
        ...
        if self.average_remeshing:
            self._ExecuteRefinement()
            # main_model_part = self.model.GetModelPart('FluidModelPart')
...
#create new settings for remeshing 
    def _compute_average_quantity(self):
        """ This method is executed in order to compute the average of a generic parameter, between consecutives time steps
        
        """
        pass
        # QUANTITY = self.parameters["processes"]["auxiliar_process_list"][1]["Parameters"]["hessian_strategy_parameters"]["metric_variable"].GetString
        # current_parameter = self.node.GetSolutionStepValue(KratosMultiphysics.QUANTITY)
        # old_average = self.node.GetValue(KratosMultiphysics.QUANTITY)
        # current_time = self.main_model_part.ProcessInfo[KratosMultiphysics.TIME]
        # delta_time = self.time_stepping["time_step"].GetDouble()
        # burning_time = self.parameters["problem_data"]["burning_time"].GetDouble()
        # if current_time > burning_time:
        #     new_average = (old_average * (previous_time - burning_time) + current_parameter * (current_time - previous_time)) / (current_time - burning_time)
        #     node.SetValue(KratosMultiphysics.QUANTITY, new_average)
